...
# This must be done during each boot to avoid interferring with DozenOS CLI config.
...
def setup_grub(root_dir: str, vars: dict[str, str]) -> None:
    ...
    grub_cfg_vars = f"{root_dir}/{grub.CFG_DOZENOS_VARS}"
    grub_cfg_modules = f"{root_dir}/{grub.CFG_DOZENOS_MODULES}"
    grub_cfg_menu = f"{root_dir}/{grub.CFG_DOZENOS_MENU}"
    grub_cfg_options = f"{root_dir}/{grub.CFG_DOZENOS_OPTIONS}"
...
def handle(name: str, cfg, cloud: Cloud, log, args: list) -> None:
    ...
    if not get_cfg_by_path(cfg, "dozenos_install/activated", False):
    ...
    boot_params: dict[str, str] = {
        "console_type": get_cfg_by_path(
            cfg, "dozenos_install/boot_params/console_type", "kvm"
        ),
        "serial_console_num": get_cfg_by_path(
            cfg, "dozenos_install/boot_params/serial_console_num", "0"
        ),
        "serial_console_speed": get_cfg_by_path(
            cfg, "dozenos_install/boot_params/serial_console_speed", "9600"
        ),
        "cmdline_extra": get_cfg_by_path(
            cfg, "dozenos_install/boot_params/cmdline_extra", ""
        ),
    }
    ...
    grub.sort_inodes(f"{DIR_DST_ROOT}/{grub.GRUB_DIR_DOZENOS}")
    grub.sort_inodes(f"{DIR_DST_ROOT}/{grub.GRUB_DIR_DOZENOS_VERS}")

    # check if we need to disable Cloud-init
    if get_cfg_by_path(cfg, "dozenos_install/ci_disable", False):
    ...
    if get_cfg_by_path(cfg, "dozenos_install/post_reboot", False):
        LOG.warn("Adding reboot trigger to postconfig script")
        script_file = Path(
            "/opt/vyatta/etc/config/scripts/dozenos-postconfig-bootup.script"
        )
