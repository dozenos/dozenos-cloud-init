...
class DozenosError(Exception):
    """Raised when the distro runs into an exception when setting dozenos config.
...
def set_config_interfaces_v1(config, iface_config: dict):
            ...
            for member in iface_config.get('bond_interfaces', []):
                config.set([
                    'interfaces', 'bonding', iface_name, 'member', 'interface'
                           ...
                           value=member,
                           replace=False)
            # read bonding parameters that DozenOS supports
    ...
    if iface_config['type'] == 'bridge':
        try:
            ...
            # read bridge parameters that DozenOS supports
...
def handle(name, cfg, cloud, log, _args):
    ...
    # DozenOS configuration file selection
    cfg_file_name = '/opt/vyatta/etc/config/config.boot'
    bak_file_name = '/usr/share/dozenos/config.boot.default'
    ...
    run(['/usr/libexec/dozenos/run-config-migration.py', file_name])
    with open(file_name, 'r') as f:
        config_file = f.read()
    config = ConfigTree(config_file)

    # Initialization of variables
    DEFAULT_DOZENOS_USER = 'dozenos'
    DEFAULT_DOZENOS_PASSWORD = 'dozenos'
    ...
    if not logins_configured:
        logger.debug("Adding fallback user: {}".format(DEFAULT_DOZENOS_USER))
        set_pass_login(config, DEFAULT_DOZENOS_USER, DEFAULT_DOZENOS_PASSWORD)
    ...
    network_config_dozenos: str = get_cfg_by_path(
        cfg, 'dozenos_config_options/network_config', network_config_global)
    if network_config_dozenos == 'disabled':
        logger.debug("Network-config is disabled (global/dozenos): {}/{}".format(
            network_config_global, network_config_dozenos))

    # process networking configuration data
    if netcfg and network_configured is False and network_config_dozenos != 'disabled':
    ...
    if hostname:
        ...
        set_config_hostname(config, 'dozenos', None)
