...
from cloudinit.config.cc_resizefs_dozenos import (
    _resize_btrfs,
    _resize_ext,
    _resize_ufs,
    _resize_xfs,
...
class TestResizefs(CiTestCase):
    ...
    def test_handle_noops_on_disabled(self):
        ...
        handle("cc_resizefs_dozenos", cfg, _cloud=None, log=LOG, args=[])
        self.assertIn(
            "DEBUG: Skipping module named cc_resizefs_dozenos, resizing disabled\n",
            self.logs.getvalue(),
        )
    ...
    @mock.patch("cloudinit.config.cc_resizefs_dozenos.util.get_mount_info")
    ...
    def test_handle_warns_on_undiscoverable_root_path_in_commandline(self):
        ...
        cfg = {"resizefs_enabled": True}
        exists_mock_path = "cloudinit.config.cc_resizefs_dozenos.os.path.exists"
        ...
        with mock.patch(exists_mock_path) as m_exists:
            ...
            wrap_and_call(
                "cloudinit.config.cc_resizefs_dozenos.util",
                {
    ...
    def test_handle_zfs_root(
        ...
        with mock.patch("cloudinit.config.cc_resizefs_dozenos.do_resize") as dresize:
            handle("cc_resizefs_dozenos", cfg, _cloud=None, log=LOG, args=[])
    ...
    def test_handle_modern_zfsroot(
        ...
        with mock.patch("cloudinit.config.cc_resizefs_dozenos.do_resize") as dresize:
            with mock.patch("cloudinit.config.cc_resizefs_dozenos.os.stat") as m_stat:
...
class TestMaybeGetDevicePathAsWritableBlock(CiTestCase):
    ...
    def test_maybe_get_writable_device_path_raises_oserror(self):
        ...
        with self.assertRaises(OSError) as context_manager:
            wrap_and_call(
                "cloudinit.config.cc_resizefs_dozenos",
                {
    ...
    @mock.patch("cloudinit.config.cc_resizefs_dozenos.os.path.isdir")
