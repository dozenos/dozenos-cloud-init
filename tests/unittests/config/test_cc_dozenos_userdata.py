...
from cloudinit.config import cc_dozenos_userdata
from tests.unittests.helpers import CiTestCase, mock


class TestDozenosUserdata(CiTestCase):
    def test_get_tag_nodes_discovers_template_paths(self):
        ...
        with mock.patch.object(
            cc_dozenos_userdata, "TEMPLATES_DIR", str(templates_dir)
        ):
            self.assertEqual(
                [("interfaces", "ethernet")],
                cc_dozenos_userdata.get_tag_nodes(),
            )
    ...
    def test_get_multi_nodes_accepts_trailing_metadata(self):
        ...
        with mock.patch.object(
            ...
            self.assertEqual(
                [("system", "name-server")],
                cc_dozenos_userdata.get_multi_nodes(),
            )
    ...
    def test_string_to_command_parses_set_and_delete(self):
        self.assertEqual(
            ...
            cc_dozenos_userdata.string_to_command(
        ...
        self.assertEqual(
            ...
            cc_dozenos_userdata.string_to_command("delete system host-name"),
        )

    def test_string_to_command_rejects_malformed_input(self):
        self.assertIsNone(cc_dozenos_userdata.string_to_command("show version"))
    ...
    def test_apply_command_set_replaces_ordinary_node(self):
        ...
        cc_dozenos_userdata.apply_command_set(config, [], [], command)
    ...
    def test_apply_command_set_appends_multi_node_value(self):
        ...
        cc_dozenos_userdata.apply_command_set(
    ...
    def test_apply_command_set_preserves_nested_tag_node_values(self):
        ...
        cc_dozenos_userdata.apply_command_set(config, tag_nodes, [], command)
    ...
    def test_apply_command_delete_uses_value_or_path(self):
        ...
        cc_dozenos_userdata.apply_command_delete(
            config,
            {
                "cmd_path": ["system", "name-server"],
                "cmd_value": "192.0.2.53",
    ...
    @mock.patch("cloudinit.config.cc_dozenos_userdata.get_multi_nodes")
    @mock.patch("cloudinit.config.cc_dozenos_userdata.get_tag_nodes")
    def test_apply_commands_routes_valid_commands_and_skips_malformed(
        ...
        cc_dozenos_userdata.apply_commands(
            config,
            [
                "set system host-name 'edge-a'",
                "show version",
